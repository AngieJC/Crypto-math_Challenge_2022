...
void RoundFunDec(u16 s[], u16 guessKey, int round)//轮函数,s为输入状态，roundkey为轮密钥，round为当前轮编号（从0开始记）
{
	u16 temp, Li, Ri;

	temp = s[0] & guessKey ^ s[1] ^ round;
	temp = temp << 2 & temp << 1 ^ temp;//S
	temp = LCS(temp, 3) ^ LCS(temp, 9) ^ LCS(temp, 14);//P
	//s[0] ^= temp; s[1] ^= temp & roundkey[round];
	Li = s[1] ^ (guessKey & temp);
	Ri = s[0] ^ temp;
	s[0] = Li;
	s[1] = Ri;
}
...
void Dec1r(u16 pt[], u16 ct[], u16 guessKey, int i) {
	u16 s[2];
	s[0] = ct[1]; s[1] = ct[0];
	RoundFunDec(s, guessKey, i - 1);
	pt[0] = s[1]; pt[1] = s[0];
}